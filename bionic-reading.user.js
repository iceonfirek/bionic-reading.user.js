...
const enCodeHTML = (s) =>
  s.replace(/[\u00A0-\u9999<>\&]/g, (w) => "&#" + w.charCodeAt(0) + ";");
...
document.documentElement.setAttribute(
  "data-site",
  location.hostname.replace(/\.\w+$|www\./gi, "")
);
...
const excludeClasses = ["highlight", "katex", "editor"];
    ...
    if (el.nodeType === 3) {
      ...
      if (el.getAttribute) {
        if (
          ...
          excludeClassesRegexi.test(el.getAttribute("class"))
        )
          return;
...
const replaceTextSymbolModeByEl = (el) => {
  ...
  el.data = text.replace(engRegexg, (word) => {
    ...
    const a = word
      .substr(0, halfLength)
      .replace(/[a-z]/g, (w) =>
        String.fromCharCode(55349, w.charCodeAt(0) + 56717)
      )
      .replace(/[A-Z]/g, (w) =>
        String.fromCharCode(55349, w.charCodeAt(0) + 56723)
      );
    const b = word
      .substr(halfLength)
      .replace(/[a-z]/g, (w) =>
        String.fromCharCode(55349, w.charCodeAt(0) + 56665)
      )
      .replace(/[A-Z]/g, (w) =>
        String.fromCharCode(55349, w.charCodeAt(0) + 56671)
      );
    return a + b;
  });
};
...
const bionic = (_) => {
  ...
  let replaceFunc = config.symbolMode
    ? replaceTextSymbolModeByEl
    : replaceTextByEl;
